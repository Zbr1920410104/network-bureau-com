...
import { SET_BOOK_SCORE, GET_BOOK_SCORE } from '@/constants/api-constants';
...
export default Form.create({ name: 'bookReview' })(({ form }) => {
  ...
  const handleSumbitSave = (e) => {
    ...
    form.validateFields(async (err, value) => {
      if (!err) {
        ...
        const res = await proxyFetch(SET_BOOK_SCORE, value);

        setSaveDataLoading(false);

        if (res) {
          dispatch(userAction.setReviewBook(true));
        }
      }
    });
  };
