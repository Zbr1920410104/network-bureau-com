...
export default (props) => {
  const { staffUuid, staffPatentUuid, staffPatentVerifyStatus } = useSelector(
    ...
    [firstVerifyPatentUrl, setFirstVerifyPatentUrl] = useState(''),
    [secondVerifyPatentUrl, setSecondVerifyPatentUrl] = useState(''),
    [thirdVerifyPatentUrl, setThirdVerifyPatentUrl] = useState(''),
  ...
  const showUploadPatentModal = (firstUrl, secondUrl, thirdUrl) => {
    setFirstVerifyPatentUrl(firstUrl);
    setSecondVerifyPatentUrl(secondUrl);
    setThirdVerifyPatentUrl(thirdUrl);
  ...
  useEffect(() => {
    if (firstVerifyPatentUrl) {
      (async () => {
        setGetFileLoading(true);
        // 附件1的url处理
        const firstPreviewUrl = await proxyFetch(
          GET_FILE_URL,
          { fileUrl: firstVerifyPatentUrl },
          'GET'
        );
        ...
        if (secondVerifyPatentUrl) {
          secondPreviewUrl = await proxyFetch(
            GET_FILE_URL,
            { fileUrl: secondVerifyPatentUrl },
            'GET'
          );
        ...
        // 附件3的url处理
        let thirdPreviewUrl = '';
        if (thirdVerifyPatentUrl) {
          thirdPreviewUrl = await proxyFetch(
            GET_FILE_URL,
            { fileUrl: thirdVerifyPatentUrl },
            'GET'
          );
  ...
  }, [firstVerifyPatentUrl, secondVerifyPatentUrl, thirdVerifyPatentUrl]);
